...
async function processWallet(wallet) {
  ...
  const minSw = settings.TIMES_SWAP[0];
  const maxSw = settings.TIMES_SWAP[1];

  const numSwaps = Math.floor(Math.random() * (maxSw - minSw + 1)) + minSw;
  ...
  for (let i = 1; i <= numSwaps; i++) {
    ...
    const nonSusdtBalances = await Promise.all(
      nonSusdtTokens.map(async (t) => ({
        token: t,
